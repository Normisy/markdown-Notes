...
策略并不是一成不变的，一般会从一个起始状态开始持续更新，从而找到一个最佳的策略
...
这里R(s)是行动过后立刻得到的即时奖励，第二项则是接下来可能达到的所有不同的未来状态$s^{'}\in S$的价值（未来奖励的期望值，使用条件概率加权），该公式是之后Bellman方程的基础
...
（这是一个不严谨的写法，s代表任意状态，这里的策略是确定性的映射$\pi:S \rightarrow A$）
即在所有的策略$\pi$中，寻找一个映射$\pi:S \rightarrow A$，使得每个状态s下，动作都能够最大化价值函数。在正式学习如何计算它的时候，作为一个结论，需要知道：
1. 一个MDP过程中，存在唯一的**最优价值函数**（请注意，最优策略不唯一），即无论从哪种策略开始优化，最终收敛到的价值函数都是同一个（不动点）
2. 对于无限时间范围的MDP问题，最优策略是确定性的，而非一个动作的概率分布
3. 同时，无限时间范围的MDP中策略具有平稳性：它不随时间变化，只取决于你的当前状态，而不取决于时间，因此我们称它是平稳的
### 策略搜索
策略搜索是一个计算最佳策略的方式，符合我们所期望的“一个理想情况下具有单调优化能力的方法“。策略搜索的目标是减少识别最优策略所需的计算量，而不必遍历全部的$|A|^{|S|}$
算法的思想是交替使用一个“可能是最优的候选决策策略”，并检查它是否可以改进，如果可以改进，就使用改进后的策略，反之停止。
粗略来说，我们期望的流程是：
1. 从i = 0开始
2. 对于所有状态s。，随机初始化它们的策略$\pi_0(s)$，即对每个策略都任意选择一个确定的动作
3. 当i == 0时或者$||\pi_i-\pi_{i-1}||_1>0$（L1范数，测量每个状态的策略是否发生了变化，它大于0说明策略发生了改变）时
	1. 使用某种价值函数$V^{\pi_i}\leftarrow MDP$来评估策略$\pi_i$，计算出可以改进的策略$\pi_{i+1}$
	2. 使用$\pi_{i+1}$改进策略$\pi_i$
	3. 令i=i+1

#### Q函数及其单调性
为了能够进行对策略的评估和改进，需要定义Q函数（动作-状态价值函数）：
$$ Q^{\pi}(s,a)=R(s,a)+\gamma \sum\limits_{s^{'}\in S}P(s^{'}|s,a)V^{\pi}(s^{'})$$
它的含义是，当我们在状态s下先执行了任意一个动作a之后（a是一个变量），然后一直跟随策略$\pi$选择固定动作，整个系统未来状态的奖励的折扣总和是多少
Q函数可以理解为：在持续执行策略$\pi$的最开始时，执行了一个动作a用于**试探**，看它是否能导致一个最佳的价值

因此在上述的算法中，对于一个特定的策略$\pi_i$，我们计算它的Q函数值（评估）：
对于所有$s\in S$和$a\in A$，$Q^{\pi_i}(s,a)=R(s,a)+\gamma \sum\limits_{s^{'}\in S}P(s^{'}|s,a)V^{\pi_{i}}(s^{'})$
对于所有$s\in S$，计算新的改进策略：
$$ \pi_{i+1}(s)=\arg\max_a Q^{\pi_{i}}(s,a)\quad \forall s\in S$$
它代表了对$\pi_i$策略进行改进后的策略，遍历了所有a，找到了那个能够使得未来奖励最大的试探动作
...
&=V^{\pi_i}(s)
\end{align}$$
可以发现，如果在Q函数计算中，若将行动a取为$\pi_i(s)$，那么这个Q函数就退化成了$V^{\pi_i}(s)$，显然有$Q^{\pi_i}(s,a)$的最大值大于等于$a$恒等于$\pi_i(s)$情况下的Q值，因此这就保证了选用新策略在每个情况下的价值至少不低于选用老策略，保证了优化的单调性：**优化后的策略至少会比优化前的策略更好或一样好**
