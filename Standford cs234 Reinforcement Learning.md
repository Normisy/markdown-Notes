...
它代表了对$\pi_i$策略进行改进后的策略，遍历了所有a，找到了那个能够使得未来奖励最大的动作
...
&=V^{\pi_i}(s)
\end{align}$$
可以发现，如果在Q函数计算中，将行动a一直固定为$\pi_i(s)$，那么这个Q函数就退化成了$V^{\pi_i}(s)$，显然有$Q^{\pi_i}(s,a)$的最大值大于等于$a$恒等于$\pi_i(s)$情况下的Q值，因此这就保证了选用新策略在每个情况下的价值至少不低于选用老策略，保证了优化的单调性
