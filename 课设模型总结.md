...
**目的**：量化餐厅的步行友好程度，以餐厅为中心，为网络的每条边（街道段）计算步行的行程时间，统计出能够到达多少个结点

**算法**：基于时间约束的网络子图提取 (Ego Graph)，提取以每个餐厅为中心，步行十分钟可以到达范围的网络子图，计算结点数
...
**作用**：评估餐厅周边可达性，分数越高表示步行可达范围越大

### 3.4 驾车可达性：统计可达面积，等时圈生成

**目的**：由于驾车等时圈包含大量节点（数千个），直接可视化困难，因此我们计算从餐厅出发，在给定时间内可到达的**空间范围面积**，即等时圈
...
**算法**：网络介数中心性 (Betweenness Centrality)，是一种对大型网络的近似优化算法，加快分析效率
```python
# network.py
...
**方法**：One-Hot Encoding，每个菜系标签是一个特征，值为0代表具有该特征，值为1代表没有该特征。数据中以"cat_”开头的字段就是这样的特征
...
**方法**：半径搜索 + 距离矩阵，查看邻域内餐厅的数量和平均评分，计算竞争强度
...
**目的**：使用基础商业属性建立预测基准模型，使用最简单的**线性回归模型**，这个模型代表“最基本的情况下，对餐厅的预测”
...
**目的**：用高级特征（网络、主题、竞争）捕捉非线性关系，即查看不能被基准模型解释的反例，让LightGBM模型使用高级特征来解释这些反例的产生原因

**算法**：梯度提升决策树LightGBM + 分位数回归
