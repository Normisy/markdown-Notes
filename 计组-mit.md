...
如果设置两个阈值$V_L$和$V_H$，规定小于等于$V_L$的为0，大于等于$V_H$的为1，在它们之间的电压值全部是没有意义的值，不能被解释为0或1
这样就可以通过例如使用高增益运算放大器（输入信号的微小变化会导致输出信号的显著变化）和基准电压来判定是否高或低于阈值，从而构建一个快速但不完美的电压到位的转换器
![[Pasted image 20260302214635.png]]
参考电压不需要特别精确，使用低成本的10%精度电阻构建的分压器即可，无论环境如何影响，wo'men'zhi'xu'ya
