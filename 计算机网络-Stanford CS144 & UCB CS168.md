...
1. **IP地址**：**网络层**所使用的地址，网络层需要它来将数据包传递给目的计算机
2. **TCP端口**：它告诉**目的主机上的软件**应该将数据传递到哪个应用程序中
...
在第2节和3.1节的时候我们看到了IPv4数据包头中用于“解复用处理”的字段，即**端口号**的存在，上面的这张图就很好地说明了它们的作用：在终端主机**自底向上接收数据包**时，在网络层之上的传输层、应用层，都可能有**不止一个的可选路径**，
