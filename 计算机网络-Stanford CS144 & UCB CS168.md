...
### 3.4.1 基本性质
链路的一个基本衡量指标是**带宽**，它决定了该链路每单位时间可以发送多少比特的数据，或接收多少比特的数据，即**单位时间内链路传输的数据量（以比特计）**，单位是bps（比特/秒），Mbps（兆比特/秒）。\[注意不是字节，而是比特（二进制位数），差了8位]，用于衡量链路的“宽度”

另一个指标是**传播延迟**，即**一个比特从发送的那一刻起到离开该链路被另一个设备接收需要多少时间**，用于衡量链路的“长度”，单位就是s秒
...
数据包的大小 / 链路带宽 也叫做**传输延迟**

链路的带宽和传播延迟数据用于衡量链路的不同方面的性能，一个链路是否好无法取决于单一指标，而取决于你要发送数据的类型和需求
- 若发送的数据包很小，带宽（传输延迟）对它来说就不是性能瓶颈，性能主要取决于传播延迟，需要传播延迟更低的链路
- 若数据包大到需要花很多时间将其塞入链路中，即传输延迟为主要瓶颈，则应该选择带宽更大的
