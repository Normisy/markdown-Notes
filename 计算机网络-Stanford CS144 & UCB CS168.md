...
尽力而为的服务是不能被直接使用的，我们需要进行重新发送/顺序组织等功能以确保数据能够按照预想地到达目的地，这就是传输层的各项协议需要保证的事情了
...
2. 域间路由协议（Inter-domain routing protocols）：不考虑每个本地网络内部的情况，而是关心将网络相互连接以形成全球互联网的协议，也称为**外部网关协议EGPs**，所有网络必须使用相同的域间路由协议以彼此链接，这个共同的协议就是==**边界网关协议BGP**==
值得注意的是，BGP也可以用于单一网络内部，实际上的实现很多也未必完全遵守这样的划分

另一个角度，按照协议的运行方式，可以将路由协议划分为：
1. 基于距离向量的协议：例如路径向量协议
2. 基于链路状态的协议
这些概念后续会详细介绍

### 3.5.2 基于目的地的转发技术
互联网使用**基于目的地的转发技术**： 当路由器执行转发决策时，无视数据包的源地址，只关心它的目的地址，因此对于同一个路由器，只要数据包的目的地址相同，选择的链路就是相同的

如第3.1节所述，每个路由器维护一个**转发表**，对于每个目的地址，路由器遵循**最长前缀匹配**原则：路由器会选择与目的IP地址匹配度最高（前缀最长）的条目。只有当所有具体条目都不匹配时，使用默认路由（前缀长度为0）。
![[Pasted image 20260111231138.png]]
在现实的低层次物理路由器上编程实现时，其形式都是以物理端口实现的，即“下一跳”就是一根插在物理端口上的电线或无线电，使用**端口号**来区分不同物理端口上的线缆，如上图所示

### 3.5.3 路由和转发
**路由和转发是不同的两个概念**
1. 转发Forwarding是单个路由器**在接收到数据包**后，根据目的地址和转发表将其发送至下一跳，使其更接近目的地的过程，只取决于数据包和本地的转发表
2. 路由Routing指的是**路由器与其他路由器进行通信，以==构建转发表==的过程**。整个过程是全局的，需要了解所有目的地的相关信息，且每当网络发生改变就需要进行一次路由（例如一个链路失效时）

因此，如何构建转发表的算法是重点，需要从全局的角度进行思考，查看每个路由器的转发表状态（全局状态），分析它们是否真正“正确”，即路由器能够正确地完成“将数据包转发至目的地”的工作，这需要**两个条件**：
1. 转发链中没有出现**死路**，即**抵达一个找不到下一跳的非目的地的路由器**，该路由器不知道该将数据包转发到哪个链路中，因此直接丢弃
![[Pasted image 20260112223951.png]]
2. 转发链中没有出现**循环**，即数据包在一组路由器之间循环传输，注意，每个路由器对同一个目的地址的转发策略是相同的，因此一旦有循环，哪怕有逃出循环的链路，也无法离开这个循环
![[Pasted image 20260112224014.png]]

定理：**路由器组成的图（不包括主机）”无死路“且”无环路“，是”全局状态是正确的“的充要条件**
- 必要性：这个定理的必要性证明很明显，因为一旦一个数据包的目的地址在路由器的转发过程中进入了死路或循环，而并没有成功到达目的地，此时由于转发策略基于目的地地址的特性，它们永远会被转发到死路或循环中，这个数据包永远无法到达目的地
- 充分性：而充分性则需要证明，只需要这两个条件被同时达成，任何数据包就一定能被正确地转发至目的地：若一个图（这里指路由器组成的图，不包含主机）没有环路和终端结点，那么一定能够将所有结点遍历一遍，并且找到最终连接着目的主机的那个结点

那么我们就需要获取网络中的一组表格，并检查网络中是否存在死路或循环。
最简单的方法是依次检查每个目的地址，在网络图上的每个转发表中取出目的地址为该地址的表项，绘制有向图，称为**有向传输树**，它描述了数据包从多个起点到一个目的地的传输路径。
这种有向树的特点是**每个节点只有一个出度**，因此一旦两条路径交会之后，就一致了，不会再分裂，因为只有一个出度
![[Pasted image 20260130181407.png]]

从图论的角度，这种数据结构称为**定向树（Oriented Standing Tree）**
它是一种树结构，即没有循环和断开的组件；该树结构中每个结点都一定被有向边连接，并且所有的有向边都指向根结点（目的地址）
![[Pasted image 20260130183256.png]]
