...
在VPN的例子中，假设我们正在访问公式内部的网站，我们的网络浏览器生成了一个`HTTP GET`请求，它首先被放置在TCP段中，然后嵌套在IP数据包中，此时IP的mu。
此时，由于需要保护私有网络的安全性，不能直接与内部web服务器通信（私人网络），而是将IP数据包放入一个TLS段中（前面提到的传输层安全性协议），该协议通过加密保护信息，TLS段也具有它自己的头
加密完成之后，这个TLS-IP-TCP-HTTP结构的数据包（TLS段）需要先被发送到VPN网关才能进入，因此又要将其放置在另一个TCP段和IP数据包中，流的目的是VPN网关，最后