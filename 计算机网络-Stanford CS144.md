...
2. IP是不可靠的：IP无法保证数据包会被传送到目的地，数据包的传输可能出现延迟、顺序错乱或压根没有送达，或是因为路由器的问题导致重复传送
3. IP会尽力进行传输：它不会因为传输过程中的问题就随意丢弃数据包，旨在必要时可能丢弃，如路由器的数据包队列因堵塞填满时不得不丢弃下一份到达的数据包。请注意，这时IP不会告诉源地址数据包被丢弃了，因为