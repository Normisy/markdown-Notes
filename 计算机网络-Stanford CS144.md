...
这就是非常简单的将协议封装在数据包内部的方法，网络四层模型并不是静态的，而是
虚拟私人网络VPN的用途是与你信任的网络建立一个安全的连接，例如使用TLS传输层安全性，当使用互联网发送IP数据包时，首先将流量转发到这个VPN连接中，然后由这个私人网络正常进行路由，从而使得网络资源像在私人网络中私有受保护一样，只需要关注一个服务的保护（VPN网关）
在VPN的例子中，假设我们正在访问公式内部的网站，我们的网络浏览器生成了一个`HTTP GET`请求，它被放置在TCP