...
举个例子，在学习自旋锁spinlock的时候提到，上自旋锁的时候不要打开中断，所以我们假设我们的`spin_lock(&xxx)`上锁的时候关中断，但是我们错误理解了开中断的时机：不小心在调用`spin_unlock($xxx)`的时候就开中断了。然后就会在使用的时候出现这样的问题：
```c
void os_run() {
	spin_lock(&AAA);
	spin_lock(&BBB);
...
哲学家吃饭问题（ABBA问题）则更加常见，每个人都要两个资源才能运行，并且按顺序分别申请两个资源之后再释放。但是每个人申请的资源之间重合一个资源，且形成头尾相连的资源等待链，此时就大概率发生死锁。
### 避免死锁
死锁必要条件：互斥（资源一次只能被一个进程使用）；请求与保持（请求阻塞时保持等待且不释放已有资源）；不可强制剥夺资源；循环等待（若干进程之间形成头尾相连的循环等待关系）

实际上，要对这四个必要条件进行破除并不是一个很简单的事情，并不是说想要避免就能避免的，于是我们介绍一些比较常见的死锁避免措施
#### AA-Deadlock
也就是第一个单进程死锁的案例，这种死锁类型很容易检测，及早报告、及早修复
xv6的实现非常值得参考，这也是本课程后面会详细关注的代码之一。它的自旋锁实现如下（很长），展现了各种防御性编程的技巧：(之后会特别精读讲解xv6的代码)
```c
...
acquire(struct spinlock *lk)
{
  ...
  if(holding(lk))    // 如果上锁的时候，cpu已经获得了这把锁，直接panic,第一时间找到问题
    panic("acquire");
...
#### ABBA-Deadlock解决方法：Lock-Ordering
也就是哲学家问题这样的循环型死锁。它最常用的防御思想是：**任意时刻系统中的锁都是有限的**，如果所有的线程都**按照同样的顺序获得所有的锁**的话，就可以消除“循环等待”的必要条件，避免ABBA型死锁的产生，这是操作系统中避免死锁的最常用方法
...
Lock-Ordering是正确的，并且工业界很多实现都会使用它，然而在一个巨大的项目中，例如Unix内核代码，你会发现这个ordering中涉及到的资源类型非常多，几乎无法让所有的模块开发者遵守它们的顺序，并且难以发现问题和维护
作为开发者，Rusty Russell在Unreliable Guide to Locking中更建议的做法是遵守以下的最佳实践准则：
- 好的锁应该被封装起来，即这个锁不能被模块外部的其他代码看见（尤其是上锁操作不能出现在头文件中）
- 当模块持有锁的时候，不要调用文件外其他复杂的函数，只在最简单的操作中使用锁
- 当持有一个锁的时候，不要试图去申请另外一个锁（事实上，上一条的目的也是为了这个），从而避免死锁发生的可能性
满足以上条件，能够保证每次获得锁时只获得一个，这样既不需要考虑ordering，也能保证死锁问题不再出现

## 7.3 并发bug：数据竞争
死锁其实是并发编程中最容易检测的bug，因为现象明显：系统停了。我们可能会想：如果我们一直不上锁，不就不会出现死锁问题了吗
实际上，这会带来另外一个问题：**数据竞争**

数据竞争：至少两个线程，它们同时访问同一段内存，并且至少有一个线程执行写操作，此时读和写的线程就产生了数据竞争，线程读出来或最终内存被写进去的结果随着每个线程的先后会产生不确定的不同，而且程序运行时不一定能显示出来这样的错误

实际上，Peterson算法已经告诉我们：我们写不对无锁的并发程序。在访问共享数据时，一定要用互斥锁保护好共享数据，从源头上避免数据竞争。

## 7.4 更多的并发bug
回顾实现并发控制的工具，它们主要就是以下两种：
- 互斥锁：代表原子性，一个操作的执行就是原子的，要么完全执行，要么完全不执行
- 条件变量：同步，即保证事情发生的顺序符合意料
因此，97%的并发bug都源于以下两种问题：
1. 忘记上锁：原子性违反（AV)
2. 忘记同步：顺序违反（OV）
### 原子性违反
我们在写并发程序中的一段函数的时候，很容易因为串行思维，忘记这段函数中的语句需要和其他线程并发，并且单独测试时也是正常的。例如一个if语句判断一个指针是否为空时
然而，在并发环境下，这个if语句很可能下一句就被另外一个线程的将该指针置空的语句插入了，导致这个if判断毫无作用

在一个并发程序中，这个问题可以上锁来解决；然而，当你的并发程序会涉及到操作系统中的对象的时候，有时上锁也无法解决问题
TOCTTOU攻击：例如，一个程序`sendmail`具有高权限，负责先检查一个文件是否满足要求，然后将其写入某个重要的文件中。在程序检查和写入的间隙，一个外部程序将检查通过的文件替换为另外一个文件的符号链接，再由`sendmail`写入重要的文件，这就出现了系统入侵
