...
```
这段代码利用cpu的向量化指令的内联函数（如`_mm256_load_ps`批量加载、`_mm256_add_ps`向量加法等）每次累加`x[i]`中的8个元素，这是cpu的simd，作用与ispc代码一致
`__mm256`代表256位的浮点向量，一个`float`大小为4B=32位，256位刚好就是8个float组成的向量

最后，对于一个并发实例组成的“集合”的规模，`programIndex`变量由系统设置，通常是向量的宽度，但
