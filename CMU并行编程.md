...
在此处的for循环中，i每次跳跃一个programCount的值，这意味着两次i之间的差值就是集合并发实例的数量值，此时**一次for循环就会同时使用集合中同时能够存在的所有的并发实例**
但是两次不同的循环的并发实例本质上还是各不相同的，因此这里使用
```
int idx = i + programIndex;
```
计算的是每个不同实例各自唯一的索引值，这个索引值用来访问每个实例中正在处理的数据
