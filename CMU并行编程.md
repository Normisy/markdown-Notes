...
系统中有4个并发实例，每个实例通过`i+=programCount`确定各自处理数据后储存的数组位置，因为是每个元素交错地被分配到不同的实例，因此也被称为**交错访问**
...
//后面都一样
```
与前面每轮循环确定程序实例相应的一个元素的访问方式不同，这样的数据访问称为**块状分配访问**：将数据分割为连续的块，每个程序实例在各自的块中访问并处理数据
